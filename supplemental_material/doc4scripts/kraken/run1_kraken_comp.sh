...
#PBS -j oe
#PBS -o /home/playera1/sandbox/classifier_comparison/kraken/results_run1_kraken_comp/results_run1_kraken_comp.log
...
mkdir -p "$kraken_dir/results_run1_kraken_comp"
...
find "$metagenomic_input" -name "*meta_sim.fa" -print0 | while read -d $'\0' meta_fa;do
	...
	kraken \
		...
		--threads 64 \
		--fasta-input "$meta_fa" > "$kraken_dir/results_run1_kraken_comp/$base.kraken"
