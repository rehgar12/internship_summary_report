#PBS -N countstoTaxIDs_CLR
#PBS -m ae
#PBS -M playera1@jhuapl.edu
#PBS -l nodes=1:ppn=4
#PBS -l walltime=240:00:00
#PBS -j oe
#PBS -o /home/playera1/sandbox/classifier_comparison2/higher_tax_STATS/BLASTN/STEP4/qsub_countstoTaxIDs_CLR.log
...
#	echo -------------------------------1st - $counter
	...
	find "$path/STEP1fix/triplet_CLR" -name "*triplet.tsv" | while read pti_filepath;do
#		echo -------------------------------2nd
#		echo $pti_filepath
...
#			echo $true_gi, $tgi, $pred_gi, $pgi, $pti
			...
			if [ "$true_gi" = "$tgi" ] && [ "$pred_gi" = "$pgi" ] 
				then
#				echo -------------------------------4th
				mkdir -p "$path/STEP4/tgi_counts_CLR"
				echo $tgi,$pgi,$pti,$count >> "$path/STEP4/tgi_counts_CLR/$tgi.counts.csv"
			fi
		done < $pti_filepath
	done
done < "$path/STEP1/gi_pair_count_CLR.tsv"
