#PBS -N slow_gi2taxid_MiSeq
#PBS -m ae
#PBS -M playera1@jhuapl.edu
#PBS -l nodes=1:ppn=4
#PBS -l walltime=240:00:00
#PBS -j oe
#PBS -o /home/playera1/sandbox/classifier_comparison2/higher_tax_STATS/BLASTN/qsub_slow_gi2taxid_MiSeq.log
...
cut -f2,3 "$path/gi_pair_count_MiSeq.tsv" | sort | uniq > "$path/gi_pair_count_MiSeq_tmp.gi2taxidInput"
...
true_ti=$(cut -f2 -d$' ' "$path/cut_pairs_tmp.txt")
	find "$path" -name "gi_pair_count_MiSeq_tmp.gi2taxidInput" | while read filename;do
	echo ----------------------------2nd
