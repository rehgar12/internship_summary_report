...
#					echo ------3rd
					...
					pti_ltr_family=$(echo ${pti_ltr%|*(no rank)} | sed -e 's/^.*(order)|//' -e 's/^.*(no rank)|//' -e 's/|.*//')
					if [ "$family" = "$pti_ltr_family" ]
						then
#						echo '--------4th: '$pti_ltr_family
						...
						while read counts_line;do
							counts_taxid=$(echo $counts_line | cut -f1 -d$' ')
							counts_count=$(echo $counts_line | cut -f2 -d$' ')
							if [ "$counts_taxid" = "$pti_ltr_taxid" ]
								then
								tax_level_count=$(bc -l <<< "$tax_level_count+$counts_count")
#								echo '----------5th:'
#								echo "$processing_gi	$counts_pgi	$counts_count	$counts_taxid	$pti_ltr_family"
							fi
						done < "$path/STEP1/ID_breakout_$2/$processing_gi.pathoID.taxids"
					else
						while read counts_line;do
							counts_taxid=$(echo $counts_line | cut -f1 -d$' ')
							counts_count=$(echo $counts_line | cut -f2 -d$' ')
...
#								echo '----------5th:'
				...
				done < "$path/STEP2/leaftoRoot_$2/$processing_gi.pti-ltr"
			P=$(bc -l <<< "$tax_level_count+$FP_count")
			FN_count=$(bc -l <<< "1000-$P")
			echo "$processing_gi,$counts_taxid,$tax_level_count,$FP_count,$FN_count"
			#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@#
			#GENUS LEVEL
			elif [ "$1" = "genus" ]
				then
...
#					echo ------3rd
					pti_ltr_taxid=$(echo $pti_ltr | cut -f1 -d$' ')
					pti_ltr_genus=$(echo $pti_ltr | sed -e 's/^.*(family)|//' -e 's/^.*(subfamily)|//' -e 's/|.*//')
					if [ "$genus" = "$pti_ltr_genus" ]
						then
#						echo '--------4th: '$pti_ltr_genus
						#open counts files from STEP4; for each true_gi, assign counts for each pred_ti
						#of the gi being processed!!!!!!!!)
						while read counts_line;do
							counts_taxid=$(echo $counts_line | cut -f1 -d$' ')
...
#								echo '----------5th:'
			...
			P=$(bc -l <<< "$tax_level_count+$FP_count")
			FN_count=$(bc -l <<< "1000-$P")
			echo "$processing_gi,$pti_ltr_taxid,$tax_level_count,$FP_count,$FN_count"
			#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@#
			#SPECIES LEVEL
			elif [ "$1" = "species" ]
				then
...
#								echo '----------5th:'
	...
	done < "$path/STEP3/taxID_26.ltr"
done < "$path/STEP3/gi_taxid_pairs.tsv"
